...
def get_movies_by_given_movie_names(movie_names):
    ...
    for movie_obj in movie_objs:
        ...
        list_of_movies.append({
            ...
            "total_number_of_ratngs":rate_sum
        })
...
def get_movies_released_in_summer_in_given_years():
    x=Movie.objects.filter(release_date__month__in=[5,6,7],release_date__year__range=(2006,2009)).values_list('movie_id',flat=True)
...
def get_movie_names_with_ratings_above_given_minimum():
    x=Rating.objects.filter(Q(movie__release_date__year__range=(2001,2100)),(Q(rating_five_count=500)|Q(rating_four_count=1000)|Q(rating_three_count=2000)|Q(rating_two_count=4000)|Q(rating_one_count=8000))).values_list('movie__name',flat=True)
    return list(x)
...
def get_actor_names_debuted_in_21st_century():
    x=Movie.objects.filter(cast__is_debut_movie=True,release_date__year__gt=2000,release_date__year__lte=2100).values_list('actors__name',flat=True).distinct()
    return list(x)     
