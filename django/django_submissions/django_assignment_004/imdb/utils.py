...
def get_female_cast_details_from_movies_having_more_than_five_female_cast():
    ...
    movie_objs=Movie.objects.annotate(count=female_count).prefetch_related('cast_set').filter(count__gt=5)
    return get_movies_by_given_movie_objs(movie_objs)

#task7 5
def get_actor_movies_released_in_year_greater_than_or_equal_to_2000():
    a=Actor.objects.prefetch_related('movie_set','movie_set__cast_set').filter(movie__release_date__year__gte=2000)
    list_of_movies=[]
    for actor in a:
        for movie_obj in actor.movie_set.all():
            #cast_obj=Cast.objects.filter(movie=movie_obj)      
            cast_list=[]
